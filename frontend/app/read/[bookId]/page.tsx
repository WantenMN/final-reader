...
export default function ReadPage() {
  ...
  useEffect(() => {
    async function fetchBookData() {
      ...
      try {
        // Fetch chapters
        const chaptersResponse = await fetch(`${API_URL}/api/books/${bookId}/chapters`);
        ...
        try {
          const stateResponse = await fetch(`${API_URL}/api/book/${bookId}/state`);
          if (stateResponse.ok) {
          ...
          console.warn('No saved reading state found or failed to fetch state:', stateError);
    ...
    async function fetchChapterContent() {
      ...
      try {
        const contentResponse = await fetch(
          `${API_URL}/api/books/${bookId}/chapter?path=${encodeURIComponent(chapter.path)}`
        );
  ...
  if (loading && chapters.length === 0) {
    return <div className="container mx-auto px-4 py-8">Loading book...</div>;
  }
